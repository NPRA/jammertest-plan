\documentclass[a4paper]{book}

\usepackage[table,x11names,dvipsnames]{xcolor}
\usepackage{tabularray}
\usepackage{pdflscape}

%% Fix so sections/chapters are manually numbered
\usepackage{tocloft}
\renewcommand\numberline[1]{}
\setcounter{secnumdepth}{0}
%\setcounter{chapter}{0}

% remove "Chapter N" at the start of chapters:
\usepackage{titlesec}
\titleformat{\chapter}[display]
  {\normalfont\bfseries}{}{0pt}{\Huge} 

% shrink margins:
\usepackage[left=.5cm, right=.5cm, bottom=2cm, top=2cm]{geometry}

% remove "Chapter N" crap in page header/footer
\usepackage{fancyhdr}
\pagestyle{fancy}
\renewcommand{\chaptermark}[1]{\markboth{#1}{}}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrule}{\hbox to \headwidth{\color{blue}\leaders\hrule height \footrulewidth\hfill}}
\renewcommand{\footrulewidth}{1pt}
\fancyfoot[RE]{\leftmark}
\fancyfoot[LO]{\leftmark}
\fancyfoot[LE,RO]{\thepage}


%% Stop text from indenting
\setlength\parindent{0pt}


%% Graphics are stored in the graphics path
\usepackage{graphicx}
\graphicspath{ {./Latex} }

%% Making hyperlinks in the table of contents
\usepackage{hyperref}
\hypersetup{
    colorlinks,
    citecolor=black,
    filecolor=black,
    linkcolor=blue,
    urlcolor=black
}

\usepackage{titlepic}
\usepackage{datetime2}

%\titlepic{\includegraphics[scale=0.2]{graphics/jampin.png}}
\title{\includegraphics[scale=0.2]{graphics/jampin.png}\\ Jammertest 2024 \\ \huge{Transmission Plan}}
\author{Jammertest Consortium}
\date{\today \\ \DTMcurrenttime}

\begin{document}
\maketitle

\newgeometry{left=3cm,right=2.5cm,top=2cm,bottom=2.5cm}
\tableofcontents
\restoregeometry

\colorlet{lightlightgray}{lightgray!30!}
\begin{landscape}

\chapter{Monday}
\Large Location 1 Bleik \\
Monday morning before lunch will be used for setting up equipment and checking that all systems are operational. After lunch the testing will start with high power jamming.
%\addcontentsline{toc}{chapter}{Monday}
%actions:
\include{./Latex/transmissionplan/plan-monday}
%local:
%\include{plan-monday}

\chapter{Tuesday}
\Large Location 1 Bleik \\
Tuesday will be used for meaconing and unintentional RFI test. During the evening there will be very high power jamming running untill 22:00.
%actions:
\include{./Latex/transmissionplan/plan-tuesday}
%local:
%\include{plan-tuesday}

\chapter{Wednesday}
\Large Location 1 Bleik \\
Wednesday will be used for position and SBAS spoofing. 
%actions:
\include{./Latex/transmissionplan/plan-wednesday}
%local:
%\include{plan-wednesday}

\chapter{Thursday}
\Large Location 1 Bleik \\
Thursday will be used for time spoofing. During the evening there will be long time meaconing running untill 22:00.
%actions:
\include{./Latex/transmissionplan/plan-thursday}
%local:
%\include{plan-thursday}

\chapter{Friday}
\Large Location 1 Bleik \\
Friday will be used for time spoofing and testing with invalid ephemerides. Time after lunch will be used for tearing down and packing up equipment.
%actions:
\include{./Latex/transmissionplan/plan-friday}
%local:
%\include{plan-friday}

\end{landscape}
\end{document}